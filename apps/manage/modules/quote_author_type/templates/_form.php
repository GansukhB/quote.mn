...
<form action="<?php echo url_for('quote_author_type/'.($form->getObject()->isNew() ? 'create' : 'update').(!$form->getObject()->isNew() ? '?id='.$form->getObject()->getId() : '')) ?>" method="post" <?php $form->isMultipart() and print 'enctype="multipart/form-data" ' ?>>
    ...
    <tfoot>
      <tr>
        <td colspan="2">
          ...
          <a href="<?php echo url_for('quote_author_type/index') ?>" class="btn btn-primary"><i class="fa fa-arrow-left"></i> Жагсаалт руу буцах</a>
          <?php if (!$form->getObject()->isNew()): ?>
            <?php echo link_to('<i class="fa fa-trash-o"></i> Устгах', 'quote_author_type/delete?id='.$form->getObject()->getId(), array('method' => 'delete', 'confirm' => 'Устгах уу?', 'class'=>'btn btn-danger')) ?>
          <?php endif; ?>
